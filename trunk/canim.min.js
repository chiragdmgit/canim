(function()
{var props=('backgroundColor borderBottomColor borderBottomWidth borderLeftColor borderLeftWidth '+'borderRightColor borderRightWidth borderSpacing borderTopColor borderTopWidth bottom color fontSize '+'fontWeight height left letterSpacing lineHeight marginBottom marginLeft marginRight marginTop maxHeight '+'maxWidth minHeight minWidth opacity outlineColor outlineOffset outlineWidth paddingBottom paddingLeft '+'paddingRight paddingTop right textIndent top width wordSpacing zIndex').split(' ');var canim=window.canim=function(el,css,opts)
{el=typeof el=='string'?document.getElementById(el):el;if(typeof css=='string')
{var stl=css;css={style:stl,klass:''};}
else css=css||{};opts=opts||{};var cssstyle=css.style,cssclass=css.klass,duration=opts.duration||1000,callback=opts.callback,easing=opts.easing||function(pos){return pos;},target=manipulate(css),current={},starttime=(new Date()).getTime(),finishtime=starttime+duration,interval,compS=((document.defaultView&&document.defaultView.getComputedStyle)?document.defaultView.getComputedStyle(el,null):el.currentStyle),compStyle={};for(prop in target)
{if(compS[prop]=='auto')
{if(target[prop])if(target[prop].v>0)compStyle[prop]=(target[prop].v/2)+target[prop].u;}
else compStyle[prop]=compS[prop];current[prop]=parse(compStyle[prop]);}
...
for(var cstyle in kstyle)style+=kstyle[cstyle]+';';style=(style+css.style).toLowerCase();tmpdiv=document.createElement('div');tmpdiv.innerHTML='<div style="'+style+'"></div>';finalcss=tmpdiv.childNodes[0].style;while(p--)if(finalcss[props[p]])tstyle[props[p]]=parse(finalcss[props[p]]);return tstyle;}
function parse(val)
{var p=parseFloat(val),q=val.replace(/^[\-\d\.]+/,'');return isNaN(p)?{v:q,f:calcolor,u:''}:{v:p,f:calvalue,u:q};}
